...
export function useStreamingResponses({
  ...
  const streamModel = async (modelId: string) => {
    ...
    try {
      ...
      const clientMode =
        process.env.NEXT_PUBLIC_MOCK_MODE === "true" ? "mock" : mode;

      if (!registry.hasProvider(model.provider as ProviderName, clientMode)) {
        throw new Error(
          `Provider ${model.provider} not available in ${clientMode} mode`,
        );
      }

      const providerClient = registry.getProvider(
      ...
      const apiKey =
        currentApiKeys[model.provider as keyof typeof currentApiKeys]?.key;
  ...
  useEffect(() => {
    ...
    const triggerApiCalls = async () => {
      await Promise.all(
        selectedModels.map(
          async (model: { id: string; provider: string; model: string }) => {
