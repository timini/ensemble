import { buildCritiquePrompt, buildRebuttalPrompt, buildJudgmentPrompt, parseJudgmentVote, calculateBranchValidity, } from './councilUtils';
/** N*(N-1) parallel critique calls — each model critiques every other model */
export async function runCritiqueRound(branches, prompt, findParticipant, completePrompt) {
    const tasks = [];
    for (const target of branches) {
        for (const critic of branches) {
            if (critic.modelId === target.modelId)
                continue;
            ...
            tasks.push(completePrompt(participant.provider, participant.modelApiId, critiquePrompt)
                .then((content) => ({
...
export async function runRebuttalRound(branches, prompt, findParticipant, completePrompt) {
    const tasks = branches.map(async (branch) => {
        if (branch.critiques.length === 0)
            return;
...
export async function runJudgmentRound(branches, prompt, findParticipant, completePrompt, validityThreshold) {
    const tasks = [];
    for (const branch of branches) {
        for (const voter of branches) {
            if (voter.modelId === branch.modelId)
                continue;
            const participant = findParticipant(voter.modelId);
            if (!participant)
                continue;
            const judgmentPrompt = buildJudgmentPrompt(prompt, branch.modelName, branch.initialAnswer, branch.critiques, branch.rebuttal?.content ?? null);
            tasks.push(completePrompt(participant.provider, participant.modelApiId, judgmentPrompt)
                .then((output) => {
