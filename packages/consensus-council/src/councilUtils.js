...
export function buildCritiquePrompt(originalPrompt, targetModelName, targetAnswer) {
    return `
...
export function buildRebuttalPrompt(originalPrompt, originalAnswer, critiques) {
...
export function buildJudgmentPrompt(originalPrompt, branchModelName, answer, critiques, rebuttal) {
    const critiquesText = critiques
        .map((c, i) => `Critique ${i + 1}:\n${c.content}`)
        .join('\n\n');
...
export function buildCouncilSummaryPrompt(originalPrompt, rankedBranches) {
...
export function parseJudgmentVote(output) {
...
export function calculateBranchValidity(validVoteCount, totalModels, threshold) {
    if (totalModels === 0) {
...
function tryParseVoteJson(text) {
    try {
