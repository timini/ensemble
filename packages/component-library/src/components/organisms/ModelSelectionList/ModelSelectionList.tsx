...
export const ModelSelectionList = React.forwardRef<HTMLDivElement, ModelSelectionListProps>(
  (
    ...
    if (models.length === 0) {
      return (
        <div ref={ref} className="text-center py-8 text-gray-500 dark:text-gray-400">
          ...
          return (
            <div key={provider} data-testid="provider-section" className="mb-8">
              ...
              <div className="flex items-center justify-between mb-4">
                <Heading level={4} size="lg" className="text-gray-900 dark:text-gray-100">{PROVIDER_LABELS[provider]}</Heading>
                {providerStatus?.[provider] && (
                  <div className="flex items-center gap-2">
                    {providerStatus[provider] === 'Ready' ? (
                      ...
                      <>
                        <span className="text-sm text-gray-600 dark:text-gray-400">{providerStatus[provider]}</span>
                        {onConfigureApiKey && (
                          <button
                            onClick={() => onConfigureApiKey(provider)}
                            className="px-3 py-1 text-xs font-medium text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950 hover:bg-blue-100 dark:hover:bg-blue-900 rounded border border-blue-200 dark:border-blue-800 transition-colors"
                          >
                            Configure
                          </button>
                        )}
